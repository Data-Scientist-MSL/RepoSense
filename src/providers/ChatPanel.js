...
class ChatPanel {
    constructor(panel, extensionUri, ollamaService) {
        this._disposables = [];
        this._conversation = [];
        this._gaps = [];
        this._summary = {};
    ...
    static async createOrShow(extensionUri, ollamaService, gaps, summary) {
        ...
        const panel = vscode.window.createWebviewPanel('reposenseChat', 'RepoSense AI Assistant', column || vscode.ViewColumn.Two, {
            enableScripts: true,
...
How can I assist you today?`,
    ...
    async _handleUserMessage(text) {
        try {
            ...
            await (0, ErrorHandler_1.handleError)(error instanceof Error ? error : new Error(String(error)), 'Chat Message', 'ChatPanel', { severity: 'error', retryable: true });
            const errorMessage = {
                role: 'assistant',
