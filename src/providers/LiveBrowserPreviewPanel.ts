...
export class LiveBrowserPreviewPanel {
  private static currentPanel: LiveBrowserPreviewPanel | undefined;
  private readonly panel: vscode.WebviewPanel;
  private readonly extensionUri: vscode.Uri;
  private testState: LiveTestState;
  ...
  public static createOrShow(
    ...
    LiveBrowserPreviewPanel.currentPanel = new LiveBrowserPreviewPanel(
      panel,
      extensionUri,
      gap,
      baseUrl
  ...
  private resetTest(gap: DetectedGap, baseUrl: string) {
    this.testState = {
      status: 'idle',
      currentStep: 0,
      totalSteps: 0,
  ...
  public updateStatus(status: LiveTestState['status']) {
    this.testState.status = status;
    this.update();
  }

  public addStep(step: Omit<LiveTestStep, 'stepNumber' | 'timestamp'>) {
    const newStep: LiveTestStep = {
      ...step,
  ...
  public updateCurrentStep(updates: Partial<LiveTestStep>) {
    if (this.testState.steps.length === 0) return;
  ...
  public addScreenshot(screenshot: string) {
    this.testState.screenshots.push(screenshot);
    this.update();
  }

  public addNetworkLog(log: NetworkLog) {
    this.testState.networkLogs.push(log);
    this.update();
  }

  public setAgentThinking(thinking: string) {
