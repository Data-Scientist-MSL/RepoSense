...
import { ReportBuilder, Report } from './ReportBuilder';
import { DiagramBuilder, DiagramsIndex } from './DiagramBuilder';
...
export class ArtifactWriter {
  ...
  constructor(workspaceFolder: string) {
    this.runStorage = new RunStorage(workspaceFolder);
   ...
   */
  async writeAllArtifacts(runId: string, analysisResult: AnalysisResult): Promise<void> {
    try {
   ...
   */
  async verifyArtifacts(runId: string): Promise<{
    scan: boolean;
    graph: boolean;
    report: boolean;
    diagrams: boolean;
