...
class GraphBuilder {
     ...
     */
    buildGraph(analysisResult) {
        // Extract endpoints
        const endpointNodes = analysisResult.endpoints.map(ep => this.createEndpointNode(ep));
        // Extract calls
        const callNodes = analysisResult.apiCalls.map(call => this.createCallNode(call));
     ...
     */
    generateStableId(type, method, normalizedPath, line) {
     ...
     */
    generateEdgeId(callId, endpointId) {
     ...
     */
    findMatchingEndpoints(callNode, endpoints) {
