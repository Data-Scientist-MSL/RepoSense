...
export class ReportGenerator {
    ...
    private calculateMetrics(gaps: GapItem[], summary: any): ReportMetrics {
        ...
        const totalMinutes = gaps.reduce((sum, gap) => {
            if (gap.severity === 'CRITICAL') return sum + 30;
            if (gap.severity === 'HIGH') return sum + 20;
            if (gap.severity === 'MEDIUM') return sum + 10;
            return sum + 5;
        }, 0);
    ...
    private async extractKeyFindings(gaps: GapItem[]): Promise<string[]> {
        ...
        if (critical.length > 0) {
            findings.push(
                `${critical.length} critical issue${critical.length > 1 ? 's' : ''} requiring immediate attention`
            );
        }

        if (high.length > 0) {
            findings.push(
                `${high.length} high-priority issue${high.length > 1 ? 's' : ''} impacting functionality`
            );
        }
        ...
        if (orphaned.length > 0) {
            ...
            findings.push(
                `${orphaned.length} orphaned API calls found, primarily in ${topFiles.slice(0, 2).join(', ')}`
            );
        }
        ...
        if (unused.length > 0) {
            findings.push(
                `${unused.length} unused backend endpoints detected, potential for code cleanup`
            );
        }
    ...
    private async generateRecommendations(gaps: GapItem[], summary: any): Promise<string[]> {
        ...
        if (critical.length > 0) {
            recommendations.push(
                `Address ${critical.length} critical gap${critical.length > 1 ? 's' : ''} immediately to restore functionality`
            );
        }
        ...
        if (orphaned.length > 5) {
            recommendations.push(
                'Implement missing backend endpoints using the auto-generated scaffolding'
            );
        }
        ...
        if (unused.length > 3) {
            recommendations.push(
                'Review and remove unused endpoints to reduce attack surface and maintenance burden'
            );
        }
        ...
        if (gaps.length > 10) {
            recommendations.push(
                'Integrate RepoSense into CI/CD pipeline to prevent future gaps'
            );
        }

        recommendations.push(
            'Generate and run automated E2E tests to validate frontend-backend integration'
        );
...
<body>
    ...
    private generateGapTableHTML(gaps: GapItem[]): string {
    ...
    private getTopFiles(gaps: GapItem[]): string[] {
    ...
    private groupBySeverity(gaps: GapItem[]): Record<string, GapItem[]> {
