...
export interface AgentStep {
  ...
  goal: string;
  thinking: string;
  result: 'pending' | 'success' | 'failed' | 'running';
  screenshot?: string;
  duration?: number;
  details?: string;
}
...
export interface UIUXIssue {
  id: string;
  type: 'accessibility' | 'usability' | 'visual' | 'performance' | 'functional';
  ...
  asIs: {
    ...
    mockup?: string; // Base64 SVG mockup
  };
  toBe: {
    description: string;
    mockup: string; // Base64 SVG mockup
...
export class UIUXAnalyzer {
  ...
  async analyzeTestResults(
    ...
    const issues = await withRetry(
      () => this.detectUIUXIssues(testResult),
      { maxAttempts: 3 }
    );
 
    // Generate mockups for each issue
    for (const issue of issues) {
      issue.toBe.mockup = this.sanitizeSVG(await withRetry(
        () => this.generateMockup(issue),
        { maxAttempts: 3 }
      ));
   ...
   */
  private async detectUIUXIssues(
    testResult: AgenticTestResult
  ): Promise<UIUXIssue[]> {
   ...
   */
  private async analyzeStep(step: AgentStep): Promise<UIUXIssue[]> {
    const issues: UIUXIssue[] = [];
   ...
   */
  private hasAccessibilityIssue(step: AgentStep): boolean {
    const indicators = [
      'no label',
      'missing alt',
      'low contrast',
    ...
    return indicators.some(indicator => 
      step.thinking?.toLowerCase().includes(indicator) ||
   ...
   */
  private createAccessibilityIssue(step: AgentStep): UIUXIssue {
    return {
...
/>`,
   ...
   */
  private hasUsabilityIssue(step: AgentStep): boolean {
    const indicators = [
      'difficult to find',
      'unclear',
      'confusing',
   ...
   */
  private createUsabilityIssue(step: AgentStep): UIUXIssue {
    return {
...
.btn-secondary {
   ...
   */
  private hasVisualIssue(step: AgentStep): boolean {
    const indicators = [
      'inconsistent',
      'misaligned',
      'poor spacing',
   ...
   */
  private createVisualIssue(step: AgentStep): UIUXIssue {
    return {
...
<Form spacing={spacing.md}>
   ...
   */
  private hasPerformanceIssue(step: AgentStep): boolean {
   ...
   */
  private createPerformanceIssue(step: AgentStep): UIUXIssue {
    return {
...
>
   ...
   */
  private extractIssuesFromError(testResult: AgenticTestResult): UIUXIssue[] {
    const issues: UIUXIssue[] = [];
   ...
   */
  private async generateMockup(issue: UIUXIssue): Promise<string> {
...
<svg width="1200" height="600" xmlns="http://www.w3.org/2000/svg">
   ...
   */
  private generateAsIsMockup(issue: UIUXIssue, x: number, y: number): string {
    switch (issue.type) {
      case 'accessibility':
        return `
...
`;
   ...
   */
  private generateToBeMockup(issue: UIUXIssue, x: number, y: number): string {
    switch (issue.type) {
      case 'accessibility':
        return `
...
`;
   ...
   */
  private generateRoadmap(issues: UIUXIssue[]): RoadmapItem[] {
   ...
   */
  private async consultExpert(pack: RecommendationPack, testResult: AgenticTestResult): Promise<void> {
...
Return your response as a single concise expert insight (max 150 words).`;
    ...
    try {
        const expertInsight = await withRetry(
            () => this.ollama.generate(prompt, { system: systemPrompt }),
  ...
  private calculateScores(issues: UIUXIssue[]) {
      // Mock score calculation based on issue count/severity
      const total = issues.length;
      if (total === 0) return { accessibility: 100, usability: 100, visual: 100, performance: 100 };
