...
class ReportGenerator {
    ...
    calculateOverallRiskScore() {
        ...
        const gaps = this.graphData.nodes.filter((n) => n.type === 'GAP');
        if (gaps.length === 0) {
            return 0;
        }
        const totalScore = gaps.reduce((sum, gap) => sum + this.calculateGapRiskScore(gap), 0);
    ...
    generateReportData() {
        const gaps = this.graphData.nodes?.filter((n) => n.type === 'GAP') || [];
        const endpoints = this.graphData.nodes?.filter((n) => n.type === 'BACKEND_ENDPOINT') || [];
        // Calculate risk distribution
        const riskDistribution = {
            critical: gaps.filter((g) => g.metadata?.severity === 'CRITICAL').length,
            high: gaps.filter((g) => g.metadata?.severity === 'HIGH').length,
            medium: gaps.filter((g) => g.metadata?.severity === 'MEDIUM').length,
            low: gaps.filter((g) => g.metadata?.severity === 'LOW').length,
        };
        ...
        if (riskDistribution.critical > 0) {
            recommendations.push(`Address ${riskDistribution.critical} CRITICAL gaps immediately before deployment`);
        }
...
<body>
  <div class="container">
     ...
     */
    generate(format = ReportFormat.JSON) {
        switch (format) {
